...
void prune(raft::device_resources const& res,
           ...
           std::optional<mdspan<const DATA_T, matrix_extent<IdxT>, row_major, d_accessor>> dataset_opt
           )
{
  RAFT_LOG_DEBUG(
  ...
  const std::size_t graph_size = new_graph.shapes[0];
  size_t array_size;
  ...
  if (dataset_opt) {
    const auto dataset = dataset_opt.value();
    RAFT_EXPECTS(
